...
- [ ] **LCU + checkpoint: install only the LCU, with the checkpoint in the same folder** (Terry, 2026-09-24; an early test run probably failed on this). Today the LCU step calls `Add-WindowsPackage` once for **every** file in `PATCHES\LCU` (in name order), so on Win11 24H2 it would install checkpoint KB5043080 as a package of its own, and the Win11 ISO (26100.9168) is already past that checkpoint (26100.1742). Microsoft's method is to install only the target LCU, with every checkpoint MSU it needs in the same folder, so DISM finds and applies the checkpoint itself when (and only when) the image needs it. To do: (1) check the order and behaviour in the real logs and against Microsoft's checkpoint-CU documentation, including the early failed run; (2) change the LCU step (install.wim, WinRE and WinPE) to add only the target LCU (the newest KB in the folder / the one the download picked), leaving the checkpoint file(s) beside it and never adding them directly; (3) keep the classic behaviour for the Win10 / Server 2022 profiles, which have no checkpoints; (4) tests, then confirm on the first Win11 24H2 servicing run (2B).
