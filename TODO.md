...
- **LTSC 2019 Safe OS DU and Setup DU titles.** The Safe OS DU search returned nothing on the real catalog (see the first real dry run above). Search catalog.update.microsoft.com for `1809 Dynamic Update` (and `1809 Safe OS`), and send the exact titles of the newest Safe OS DU and Setup DU for x64, or confirm none exist any more for 1809 - then the profile's search strings and filters get set from that.
