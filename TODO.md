...
- [ ] **Languages go into install.wim only - never WinRE, never boot.wim (decision, Terry 2026-09-26).** WinRE and boot.wim (WinPE / Setup) stay English-only. The script still adds languages to both: `Service-WinRe` calls `Add-WinPeLanguages $WinReMount $OcRoot $Languages 'WinRE'` whenever WinRE servicing is ticked and languages are selected, and `Service-BootWim` does the same for boot.wim when Boot is ticked. Until this is fixed, leave WinRE and Boot unticked on runs with languages. To do: remove both language steps (WinRE keeps SSU / Safe OS DU servicing, boot.wim keeps its patching), log "WinRE / boot.wim: languages are not added (install.wim only)", drop any WinRE / WinPE language checks from verify and the WinPE-OC lookups that only served them, and add tests.
