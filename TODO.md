...
- [ ] **Languages go into install.wim only - never WinRE (decision, Terry 2026-09-26).** The script still adds them to WinRE: `Service-WinRe` calls `Add-WinPeLanguages $WinReMount $OcRoot $Languages 'WinRE'` whenever WinRE servicing is ticked and languages are selected (and `Service-BootWim` does the same for boot.wim when Boot is ticked). To do: remove the WinRE language step (WinRE keeps SSU / Safe OS DU servicing only), log "WinRE: languages are not added (install.wim only)", drop the WinRE language checks from verify if any, and add a test. **Confirm with Terry:** "install.wim only" suggests boot.wim (WinPE / Setup UI) should stay English-only too - remove that step as well?
...
- **Profile defaults.** Each profile's `defaultLanguages` should be a subset of the list. Today they are not: the built-in default set has **en-gb and zh-tw**, which are not in `Languages.json` (and the LTSC 2019 run used both). Decide with Terry: add them to the file, or change the profile defaults. A default that is not in the list gets a WARN and is not selected.
- **Entries to check before relying on them:** `en-us` is the base language of every English source image (adding its language pack is at best a no-op - skip it, or leave it out of the list); `ca-es` (Catalan) is a Language Interface Pack on Windows 10, with a different cab name and a parent language, so check it on each Language Pack ISO; the ten new languages (ca-es, cs-cz, hu-hu, pl-pl, pt-pt, ro-ro, ru-ru, sk-sk, sv-se, tr-tr) have not been in any run yet - the preflight already names any language pack missing from an ISO.
...
- 2026-09-26: decision - languages go into install.wim only, never WinRE (the WinRE-with-languages test is dropped; removing the WinRE language step is an open item). `Languages.json` committed; Languages tab to be driven by it (step 10e).
