...
| [10](#s10) | Operator UX: INSTRUCTIONS.md + Instructions tab, saved settings, utility menu, Languages tab from Languages.json | Claude (+ Terry for the inventory script) | 10c and 10e built (confirm 10c in the real GUI); 10a, 10b, 10d not started |
...
**10c. "Save settings" button: remember the GUI choices per OS (Terry, 2026-09-27; replaces the earlier auto-save idea) - built 2026-09-27; confirm in the real GUI**

**What was built:** "Save settings" and "Reset to defaults" buttons at the bottom, left of Start refresh (both ignored while a run is in progress). Engine: `Save-OsSettings` / `Read-OsSettings` / `Remove-OsSettings` for `Settings\<profile folder>.json` (the 13 checkboxes as true/false, the ticked languages as codes, plus the OS name and save time) and `Save-GeneralSettings` / `Read-GeneralSettings` for the repository root in `Settings\General.json`; `Settings\` sits beside `Profiles\`. GUI: selecting an OS (and start-up) applies its saved settings, or the window's own default ticks plus the profile's default languages when it has none - so switching from a saved OS to an unsaved one does not carry ticks over; the saved repository root is loaded at start-up; the log says what was loaded or saved. A saved "no languages" stays English only. Reset asks for confirmation, deletes the file and re-applies the defaults. Built with the plain label "Save settings" (the tooltip and the log name the OS) instead of the planned "Save settings for <OS>", to keep the button bar a fixed width - Terry to confirm or ask for the OS name on the button. Tests: 12 in `profiles.ps1` (P11: round trip, unknown option names dropped, missing language, three kinds of bad file, remove, repository root, regenerating `Profiles\` leaves `Settings\` alone) and 6 in `xaml.ps1` on the real window's controls (defaults, save, switch OS, switch back, reset).
**To confirm in the real GUI:** tick / untick a few boxes and languages for one OS, press Save settings (log: "Settings for <OS> saved to ...\Settings\<folder>.json"), switch to another OS and back, restart the tool - the choices come back; press Reset to defaults and confirm.

- **Button** at the bottom of the window (beside Run / Cancel): **Save settings for <selected OS>**. Explicit, not automatic - nothing is remembered unless Terry presses it. It saves the current choices for the OS selected in the drop-down: every checkbox on "Source and targets" and "Updates and features" (Preflight, Install, Boot, WinRE, Verify, Build media, Build ISO, SSU, LCU, Safe OS, .NET CU, Setup DU, NetFX3) and the ticked languages (as codes). The repository root is saved once, not per OS.
- **Load:** when an OS is selected (and at start-up), its saved settings are applied; an OS with no saved settings gets the XAML defaults and its profile's `defaultLanguages`, as today. The log says which: "Settings for <OS> loaded from Settings\<folder>.json" or "No saved settings for <OS>; using the defaults". A saved language that is no longer in `Languages.json` is logged and not ticked; a bad settings file is reported and ignored, never a crash.
- **Where (recommendation, not the OS profile file):** `Settings\<profile folder>.json` beside `Profiles\` (e.g. `Settings\Win10_Enterprise_LTSC_2019.json`), plus `Settings\General.json` for the repository root. Terry's first idea was to store them in the selected OS's profile `.json`; a separate file is better because: (1) the profile files are OS definitions (edition, language pack pattern, catalog search rules) that get **regenerated** - the documented fix for stale catalog rules on 2026-09-24 was "delete the Profiles folder and reload", which would silently wipe saved settings stored there; (2) writing a profile back means re-serialising it with `ConvertTo-Json`, which rewrites hand-edited formatting and risks damaging the file the whole OS depends on; (3) it keeps "what this OS is" separate from "what I usually tick", so a profile can be replaced or shared without carrying one operator's choices. The profile's `defaultLanguages` stays the shipped default that saved settings override. `Settings/` is already in `.gitignore`.
- **Reset:** a "Reset to defaults" link or button next to Save deletes that OS's settings file (after a confirmation) and re-applies the defaults. The 10d menu's **Clear Settings** clears all saved settings (every `Settings\*.json`).
- **Later reuse:** step 7's SCCM tab (site server, DP / DP group, content source) saves into the same files - `General.json` for the site server, the OS file for per-OS values - rather than a second mechanism.
- **Tests:** save then load round trip for checkboxes and languages; an OS without a file gets the defaults; a language missing from `Languages.json` is dropped with a WARN; a bad file is ignored with a WARN; Reset removes the file; regenerating `Profiles\` leaves `Settings\` untouched; a WPF check that the button saves the currently selected OS only.
...
- A small button or menu glyph in the header (near the OS/phase display added in step 3a), opening a dropdown with three actions:
  - **Clear Settings** â€” deletes every saved settings file from 10c (`Settings\*.json`) so each OS goes back to the XAML defaults and its profile's `defaultLanguages`. Confirm before doing it (destructive-ish, easy to fat-finger).
...
**10e. Languages tab driven by `Languages.json` (Terry, 2026-09-26) - built 2026-09-26; confirm in the real GUI**

**What was built:** the ten hard-coded XAML items are gone. `Get-BuiltInLanguageData` carries the 20-entry list (a test keeps it identical to the repo `Languages.json`); `Import-LanguageList` writes `Profiles\Languages.json` from it when missing (same one-entry-per-line style, no BOM), validates the file (every entry needs `language` and `code`, codes like `de-de`, no duplicates; codes are lower-cased) and falls back to the built-in list with a WARN on any problem; `Import-OsProfiles` no longer treats `Languages.json` as an OS profile (a folder holding only `Languages.json` still gets the five profile files). The GUI builds the list at start-up and on "Reload profiles": each item shows `Full name - code`, the code sits in the item's `Tag`, and selection, profile defaults and the run options use the code. `Get-DefaultLanguageSelection` pre-selects a profile's defaults that are on the list and logs a WARN for any that are not. The tab text now says languages go into install.wim only. Tests: 15 in `profiles.ps1` (P10) and 5 in `xaml.ps1`, 4 of them on real WPF controls (list built from the file, defaults pre-selected by code, a run gets codes, a missing default is logged and not selected); the real window XAML also loads in WPF on 5.1 and 7.
**To confirm in the real GUI:** start the tool, open the Languages tab: 20 entries as `Full name - code`, `Profiles\Languages.json` created beside the profile files, the selected OS's defaults pre-selected; untick one, run a Preflight, and check the log's `Languages:` line lists codes.
...
- **Profile defaults.** Each profile's `defaultLanguages` should be a subset of the list; a default that is not in the list gets a WARN and is not selected (built). As of 2026-09-26 the built-in default set (de-de, en-gb, es-es, fr-fr, it-it, ja-jp, ko-kr, pt-br, zh-cn, zh-tw) is fully in `Languages.json` - Terry added en-gb "English (United Kingdom)" and zh-tw "Chinese (Traditional, Taiwan)" and removed en-us (the base language of every English source image), 20 entries.
